...
### ✅ Everything Working (as of 2026-08-06 — avatar cropper + profile visibility for all users)
Follow-up to the profile-edit feature (below): a way to reposition/crop the picture before upload, and making profile pictures (and a general profile view) visible for regular participants, not just teachers — while keeping the existing "phone only after an accepted match" rule intact.
- **New avatar cropper** (`oraita-web/src/components/AvatarCropper.tsx`) — opens as a modal right after picking a file in `/profile`. Drag-to-reposition + zoom slider (1×–3×) inside a circular frame, using Pointer Events (covers mouse + touch with one set of handlers) and clamped offsets so the image can never show a gap. On confirm, the visible region is drawn onto an offscreen `<canvas>` and exported as a JPEG blob — so the file that actually gets uploaded to Cloudinary is already correctly cropped, meaning every existing avatar spot in the app (comments, `TeacherProfile`, participants list) needed zero changes to display it correctly. No new npm dependency.
  - Hit a real bug while building this: the cropper's image `<img>` initially showed broken (`naturalWidth: 0`) and the modal would sometimes vanish entirely. Root cause — the blob URL was created once via a `useState` lazy initializer but revoked in a `useEffect` cleanup; React 18 `StrictMode` (used in `main.tsx`) intentionally double-invokes effects in dev (mount → cleanup → mount again) to catch exactly this kind of bug, so the cleanup fired immediately and revoked the URL out from under the still-displayed image. Fixed by creating and revoking the object URL inside the same `useEffect` run (keyed on `file`) instead of splitting creation into a `useState` initializer — verified live afterward (drag, zoom, and confirm all work correctly, and the persisted Cloudinary image matches exactly what was framed in the cropper).
- **Profile pictures now show for every participant, not just teachers**: the "who's registered" list on `SingleLesson` and the match-request rows on `Dashboard` both render a small avatar (`.avatar-sm`) next to each name, with names linking to a new general profile view. Backend populates gained `profilePicture`: `lessonController.getLessonById`'s `participants` populate, and `matchRequestController.getMyMatchRequests`'s `from`/`to` populate (picture shown regardless of request status — only `phone` stays conditional on `accepted`, unchanged).
- **New general profile page** `oraita-web/src/pages/UserProfilePage.tsx`, route `/user/:id` (login-gated via `PrivateRoute`, matching the existing privacy posture for participant identity). Backed by a new `GET /api/users/:id` (`getUserById` in `userController.ts`) returning only `{ _id, name, profilePicture, gender }` by default — and additionally `phone` if (and only if) the viewer and the target have an `accepted` `MatchRequest` between them, checked server-side the same way `getMyMatchRequests` already does. This means the "phone only after an accepted match" rule now applies uniformly on the Dashboard match panel *and* this new profile page, enforced in one place rather than trusted client-side. Viewing your own `/user/:id` redirects to `/profile` instead. The page reuses `TeacherProfile`'s existing `.profile-cover`/`.profile-avatar` CSS classes (no new styling needed) and links to `/teacherprofile/:id` (which already handles the zero-lessons case gracefully).
- **Verified live** with two temporary QA accounts (`test.profile.qa@example.com`, `test.match.qa@example.com`, kept in the local dev DB) joined to the same lesson: confirmed both participants see each other's avatar in the participants list; sent a match request and confirmed the recipient's Dashboard row showed the sender's picture *before* accepting; after accepting, confirmed the phone now appears both in the Dashboard's existing accepted-contacts view (unchanged) and on the new `/user/:id` page (new); confirmed `/user/:id` for a not-yet-matched user shows the picture but no phone; confirmed visiting your own `/user/:id` redirects to `/profile`. `tsc --noEmit` clean on both `server/` and `oraita-web/`.
- **Click-to-enlarge lightbox added afterward** — new reusable `oraita-web/src/components/ImageLightbox.tsx` (dark full-screen overlay, dismiss via backdrop click, ✕ button, or Escape). Wired into the three places a large profile picture is shown: `UserProfilePage.tsx`, `TeacherProfile.tsx`, and the own-picture preview on `Profile.tsx` itself — the `.profile-avatar` circle only becomes clickable (`cursor: zoom-in`, new `.clickable` class) when a picture is actually set. Verified live on `Profile.tsx` and `UserProfilePage.tsx` (backdrop click, ✕ button, and Escape all dismiss correctly); `TeacherProfile.tsx` uses the identical pattern. `tsc --noEmit` clean.
